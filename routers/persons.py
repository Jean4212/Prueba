...
def persons(dni: str):
    ...
    path = getcwd() + "/static/uploads/"
    
    if datos.foto:
        remove(path + datos.foto)

    if datos.fotodni:
        remove(path + datos.fotodni)

    if datos.fotolicencia:
        remove(path + datos.fotolicencia)

    if datos.fotopolicial:
        remove(path + datos.fotopolicial)
